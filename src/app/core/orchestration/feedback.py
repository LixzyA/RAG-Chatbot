...
class SelfFeedbackLoop:
    ...
    async def refine_and_regenerate(
        ...
        Returns a refinement instruction string to inject into the system prompt.
        """
        strategies: list[str] = []

        if (
            evaluation.hallucination_detected
            or evaluation.faithfulness < settings.self_feedback_threshold
        ):
            strategies.append(
                "STRICT GROUNDING: Only use facts explicitly stated in the provided context. "
                "Do not add any external knowledge, speculation, or invented details.\n"
            )

        if evaluation.relevance < settings.self_feedback_threshold:
            strategies.append(
                f"FOCUS: Answer the user's specific question directly: '{query}'. "
                "Avoid tangential information.\n"
            )

        if evaluation.completeness < settings.self_feedback_threshold:
            missing = (
                ", ".join(evaluation.missing_from_context)
                if evaluation.missing_from_context
                else "relevant details"
            )
            strategies.append(
                f"COMPLETENESS: Ensure the answer covers all relevant information, "
                f"including: {missing}. Do not omit key points from the context.\n"
            )

        if not strategies:
            strategies.append(evaluation.critique)
